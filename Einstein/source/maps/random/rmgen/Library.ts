...
function getObstructionSize(templateName: string, margin: number = 0) {
 ...
 * Returns actually placed areas.
 */
function createAreasInAreas(centeredPlacer: ICenteredPlacer, painter, constraints, amount, retryFactor, areas: Area[]) {
    let placeFunc = function () {
 ...
 * Returns the number of actually placed groups.
 */
function createObjectGroupsByAreas(group, player, constraints: IConstraint[], amount, retryFactor, areas: Area[], behaveDeprecated = false) {
    let placeFunc = function () {
...
function createTerrain(terrain: string | string[]): ITerrain {
    if (typeof terrain == "string") {
        var x = terrain.split(TERRAIN_SEPARATOR);
        return new SimpleTerrain(x[0], x[1]);
    }
    else {
 ...
 */
function createArea(placer: IPlacer, painters: IPainter | IPainter[], constraints: IConstraint[] = []) {
 ...
 */
function paintTerrainBasedOnHeight(minHeight: number, maxHeight: number, mode: ElevationMode, terrain) {
    return createArea(
        new HeightPlacer(mode, minHeight, maxHeight),
        new TerrainPainter(terrain));
}

function paintTileClassBasedOnHeight(minHeight: number, maxHeight: number, mode: ElevationMode, tileClass: TileClass) {
    return createArea(
        new HeightPlacer(mode, minHeight, maxHeight),
 ...
 */
function avoidClasses(...args: (TileClass | number)[]): IConstraint {
    let ar = [];
