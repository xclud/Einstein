 ...
 */
function distanceOfPointFromLine(lineStart: Vector2D, lineEnd: Vector2D, point: Vector2D): number {
 ...
 */
function getBoundingBox(points: Vector2D[]): BoundingBox {
...
function getPointsInBoundingBox(boundingBox: BoundingBox): Vector2D[] {
    let points: Vector2D[] = [];
 ...
 */
function sortPointsShortestCycle(points: Vector2D[]) {
    let order: number[] = [];
    ...
    for (let i = 0; i < 3; ++i) {
        order.push(i);
        pointsToAdd.shift(); //TODO: pointsToAdd.shift(i);
        if (i) {
            distances.push(euclidDistance2D(points[order[i]].x, points[order[i]].y, points[order[i - 1]].x, points[order[i - 1]].y));
        }
    }
