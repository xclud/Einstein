 ...
 */
class SmoothElevationPainter implements IPainter {
    type: ElevationType;
    elevation: number;
    blendRadius: number;
    randomElevation: number;

    constructor(type: ElevationType, elevation: number, blendRadius: number, randomElevation: number = 0) {
        this.type = type;
        this.elevation = elevation;
        this.blendRadius = blendRadius;
        this.randomElevation = randomElevation;

        if (type != ElevationType.ELEVATION_SET && type != ElevationType.ELEVATION_MODIFY)
    ...
    paint(area: Area) {
        ...
        breadthFirstSearchPaint(area, heightmapSize, {
            "brushSize": brushSize,
            ...
            "paintTile": (point, distance) => {
                ...
                if (this.type == ElevationType.ELEVATION_SET)
 ...
 */
function breadthFirstSearchPaint(area: Area, gridSize: number, args) {
    // These variables save which points were visited already and the shortest distance to the area
    let saw = [];
    let dist = [];
    for (let i = 0; i < gridSize; ++i) {
        saw[i] = new Uint8Array(gridSize);
        dist[i] = new Uint16Array(gridSize);
    }

    let withinGrid = (x, z) => Math.min(x, z) >= 0 && Math.max(x, z) < gridSize;
    ...
    for (let point of area.getPoints())
        ...
        for (let dx = -1; dx < 1 + args.brushSize; ++dx) {
            ...
            for (let dz = -1; dz < 1 + args.brushSize; ++dz) {
                ...
                if (!withinGrid(nx, nz) || args.withinArea(area, position) || saw[nx][nz])
                    continue;
