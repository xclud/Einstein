class Formation extends Component {
    ...
    maxColumnsUsed: any[];
    maxColumns: number;
    formationSeparation: any;
    formationMembersWithAura: any[];
    depth: number;
    width: number;
    offsets: any;
    ...
    static readonly g_ColumnDistanceThreshold = 128; // distance at which we'll switch between column/box formations
    formationShape: any;
    shiftRows: any;
    centerGap: any;
    sortingOrder: any;
    oldOrientation: { "sin": number; "cos": number; };
    sortingClasses: string[];
    sloppyness: number;
    ...
    GetClosestMember(ent: number, filter: (entity: number) => boolean = undefined) {
     ...
     */
    GetFormationAnimation(entity, defaultAnimation) {
     ...
     */
    SetRearrange(rearrange: boolean) {
     ...
     */
    MoveMembersIntoFormation(moveCenter: boolean, force: boolean) {
        ...
        var columnar = walkingDistance > Formation.g_ColumnDistanceThreshold;
    ...
    ComputeFormationOffsets(active: any[], positions) {
        ...
        var types = {};
        for (var ii = 0; ii < sortingClasses.length; ++ii) {
            types[sortingClasses[ii]] = [];
        }

        for (var iii in active) {
            var cmpIdentity = Engine.QueryInterface(active[iii], IID_Identity);
            var classes = cmpIdentity.GetClassesList();
            var done = false;
            for (var c = 0; c < sortingClasses.length; ++c) {
                if (classes.indexOf(sortingClasses[c]) > -1) {
                    types[sortingClasses[c]].push({ "ent": active[iii], "pos": positions[iii] });
                    done = true;
                    break;
                }
            }
            if (!done)
                types["Unknown"].push({ "ent": active[iii], "pos": positions[iii] });
        }
        ...
        if (this.template.FormationName == "Scatter") {
            ...
            for (var i = 0; i < count; ++i) {
                var obj: any = new Vector2D(randFloat(0, width), randFloat(0, width));
                obj.row = 1;
                obj.column = i + 1;
                offsets.push(obj);
            }
        ...
        if (this.sortingOrder == "fillFromTheSides")
            offsets.sort(function (o1, o2) { return Math.abs(o1.x) < Math.abs(o2.x) ? 1 : 0; });
        else if (this.sortingOrder == "fillToTheCenter")
            offsets.sort(function (o1, o2) {
                return Math.max(Math.abs(o1.x), Math.abs(o1.y)) < Math.max(Math.abs(o2.x), Math.abs(o2.y)) ? 1 : 0;
            });
     ...
     */
    GetRealOffsetPositions(offsets, pos) {
        var offsetPositions = [];
    ...
    ShapeUpdate() {
        ...
        for (var i = this.twinFormations.length - 1; i >= 0; --i) {
            ...
            var cmpOtherFormation = Engine.QueryInterface<Formation>(this.twinFormations[i], IID_Formation);
    ...
    RegisterTwinFormation(entity) {
        var cmpFormation = Engine.QueryInterface<Formation>(entity, IID_Formation);
        if (!cmpFormation)
            return;
    ...
    LoadFormation(newTemplate) {
        ...
        var cmpFormation = Engine.QueryInterface<Formation>(newFormation, IID_Formation);
