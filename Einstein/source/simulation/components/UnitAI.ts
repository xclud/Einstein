class UnitAI extends Component {

    orderQueue: any[]; // current order is at the front of the list
    order: any; // always == this.orderQueue[0]
    formationController: number; // entity with IID_Formation that we belong to
    isGarrisoned: boolean;
    isIdle: boolean;
    finishedOrder: boolean; // used to find if all formation members finished the order
    ...
    lastHealed = undefined;
    losRangeQuery: any;
    losHealRangeQuery: any;
    timer: any;
    guardRange: any;
    expectedRoute: any;
    waypoints: any[];
    ...
    UnitFsmSpec = {
        ...
        "FORMATIONCONTROLLER": {
            ...
            "Order.Attack": function (msg) {
                ...
                var cmpTargetUnitAI = Engine.QueryInterface<UnitAI>(target, IID_UnitAI);
                ...
                "ESCORTING": {
                    ...
                    "MoveStarted": function (msg) {
                        ...
                        if (cmpUnitMotion.IsInTargetRange(this.isGuardOf, 0, 3 * this.guardRange)) {
                            var cmpUnitAI = Engine.QueryInterface<UnitAI>(this.isGuardOf, IID_UnitAI);
                            if (cmpUnitAI) {
            ...
            "COMBAT": {
                ...
                "CHASING": {
                    "enter": function () {
                        ...
                        this.SelectAnimation("move");
                        var cmpUnitAI = Engine.QueryInterface<UnitAI>(this.order.data.target, IID_UnitAI);
                            ...
                            if (this.CheckGarrisonRange(target)) {
                                ...
                                if (cmpGarrisonHolder.Garrison(this.entity)) {
                                    ...
                                    if (this.pickup) {
                                        ...
                                        var cmpHolderUnitAI = Engine.QueryInterface<UnitAI>(target, IID_UnitAI);
                                ...
                                if (this.pickup) {
                                    var cmpUnitAI = Engine.QueryInterface<UnitAI>(this.pickup, IID_UnitAI);
    ...
    // which can be healed.
    // This should be called whenever our ownership changes.
    SetupHealRangeQuery(enable = true) {
        var cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
    ...
    SetNextStateAlwaysEntering(state) {
    ...
    FinishOrder() {
        ...
        if (this.IsFormationMember()) {
            let cmpUnitAI = Engine.QueryInterface<UnitAI>(this.formationController, IID_UnitAI);
            if (cmpUnitAI) {
     ...
     */
    PushOrderAfterForced(type, data) {
    ...
    UpdateWorkOrders(type) {
        ...
        if (this.IsFormationMember()) {
            var cmpUnitAI = Engine.QueryInterface<UnitAI>(this.formationController, IID_UnitAI);
            if (cmpUnitAI) {
    ...
    OnGlobalConstructionFinished(msg) {
     ...
     * gatherers.
     */
    FindNearbyResource(filter, target) {
        var cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
     ...
     */
    FindNearestDropsite(genericType) {
        let cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
        if (!cmpOwnership || cmpOwnership.GetOwner() == INVALID_PLAYER)
            return undefined;
    ...
    SelectAnimation(name, once = false, speed = 1.0) {
        let cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
        if (!cmpVisual)
            return;
    ...
    SetAnimationSync(actiontime, repeattime) {
        var cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
        if (!cmpVisual)
            return;
    ...
    MoveToPointRange(x, z, rangeMin, rangeMax) {
        var cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
    ...
    MoveToTargetRange(target, iid, type?) {
     ...
     */
    MoveToTargetAttackRange(target, type) {
        // for formation members, the formation will take care of the range check
        if (this.IsFormationMember()) {
            var cmpFormationUnitAI = Engine.QueryInterface<UnitAI>(this.formationController, IID_UnitAI);
        ...
        if (h > -range.max / 2)
            var parabolicMaxRange = Math.sqrt(square(range.max) + 2 * range.max * h);
        else
    ...
    MoveToTargetRangeExplicit(target, min, max) {
        if (!this.CheckTargetVisible(target))
            return false;
    ...
    CheckPointRangeExplicit(x, z, min, max) {
        var cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
        return cmpUnitMotion.IsInPointRange(x, z, min, max);
    };

    CheckTargetRange(target, iid, type?) {
        var cmpRanged = Engine.QueryInterface(this.entity, iid);
        if (!cmpRanged)
            return false;
        var range = cmpRanged.GetRange(type);
     ...
     */
    CheckTargetAttackRange(target, type) {
        // for formation members, the formation will take care of the range check
        if (this.IsFormationMember()) {
            var cmpFormationUnitAI = Engine.QueryInterface<UnitAI>(this.formationController, IID_UnitAI);
            if (cmpFormationUnitAI && cmpFormationUnitAI.IsAttackingAsFormation()
    ...
    CheckTargetRangeExplicit(target, min, max) {
        var cmpUnitMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
        return cmpUnitMotion.IsInTargetRange(target, min, max);
    };

    CheckGarrisonRange(target) {
        var cmpGarrisonHolder = Engine.QueryInterface(target, IID_GarrisonHolder);
        if (!cmpGarrisonHolder)
            return false;
        var range = cmpGarrisonHolder.GetLoadingRange();
    ...
    CheckTargetDistanceFromHeldPosition(target, iid, type) {
        var cmpRanged = Engine.QueryInterface(this.entity, iid);
        ...
        return euclidDistance2D(pos.x, pos.z, heldPosition.x, heldPosition.z) < halfvision + range.max;
    };

    CheckTargetIsInVisionRange(target) {
        var cmpVision = Engine.QueryInterface(this.entity, IID_Vision);
        if (!cmpVision)
            return false;
    ...
    GetBestAttackAgainst(target, allowCapture) {
        var cmpAttack = Engine.QueryInterface(this.entity, IID_Attack);
        if (!cmpAttack)
            return undefined;
     ...
     */
    RespondToTargetedEntities(ents) {
        if (!ents.length)
            return false;
     ...
     * Returns true if it responded.
     */
    RespondToHealableEntities(ents) {
     ...
     */
    ShouldAbandonChase(target, force, iid, type) {
        ...
        if (this.isGuardOf) {
            var cmpUnitAI = Engine.QueryInterface<UnitAI>(target, IID_UnitAI);
     ...
     */
    ShouldChaseTargetedEntity(target, force) {
        if (this.IsTurret())
            return false;
        ...
        if (this.isGuardOf) {
            let cmpUnitAI = Engine.QueryInterface<UnitAI>(target, IID_UnitAI);
     ...
     */
    WalkToPointRange(x, z, min, max, queued) {
     ...
     */
    WalkAndFight(x, z, targetClasses, allowCapture = true, queued = false) {
        this.AddOrder("WalkAndFight", { "x": x, "z": z, "targetClasses": targetClasses, "allowCapture": allowCapture, "force": true }, queued);
    };

    Patrol(x, z, targetClasses, allowCapture = true, queued = false) {
        if (!this.CanPatrol()) {
     ...
     */
    Attack(target, allowCapture = true, queued = false) {
        if (!this.CanAttack(target)) {
     ...
     */
    PerformGather(target, queued, force) {
     ...
     */
    GatherNearPosition(x, z, type, template, queued) {
        // Remove "resource|" prefix from template name, if present.
        if (template.indexOf("resource|") != -1)
            template = template.slice(9);
     ...
     */
    SetupTradeRoute(target, source, route, queued) {
    ...
    PerformTradeAndMoveToNextMarket(currentMarket) {
     ...
     * until the target is reached
     */
    Repair(target, autocontinue, queued) {
    ...
    SetTurretStance() {
        ...
        if (this.GetStance().respondStandGround)
            return;
        for (let stance in UnitAI.g_Stances) {
            if (!UnitAI.g_Stances[stance].respondStandGround)
                continue;
    ...
    GetTargetsFromUnit() {
        ...
        var attackfilter = function (e) {
            ...
            if (cmpOwnership && cmpOwnership.GetOwner() > 0)
                return true;
            var cmpUnitAI = Engine.QueryInterface<UnitAI>(e, IID_UnitAI);
    ...
    GetStance() {
        return UnitAI.g_Stances[this.stance];
    };

    GetSelectableStances() {
        if (this.IsTurret())
            return [];
        return Object.keys(UnitAI.g_Stances).filter(key => UnitAI.g_Stances[key].selectable);
    };
    ...
    SetHeldPositionOnEntity(entity) {
        var cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
        if (!cmpPosition || !cmpPosition.IsInWorld())
            return;
        var pos = cmpPosition.GetPosition();
    ...
    CanReturnResource(target, checkCarriedResource) {
        if (this.IsTurret())
            return false;
        // Formation controllers should always respond to commands
        // (then the individual units can make up their own minds)
    ...
    SetFacePointAfterMove(val) {
        var cmpMotion = Engine.QueryInterface(this.entity, IID_UnitMotion);
        if (cmpMotion)
            cmpMotion.SetFacePointAfterMove(val);
    }

    AttackEntitiesByPreference(ents) {
        if (!ents.length)
            return false;
    ...
    CallMemberFunction(funcname, args) {
        ...
        cmpFormation.GetMembers().forEach(ent => {
            var cmpUnitAI = Engine.QueryInterface<UnitAI>(ent, IID_UnitAI);
    ...
    UnitFsm = new FSM(this.UnitFsmSpec);
}
