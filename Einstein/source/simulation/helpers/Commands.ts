...
var g_Commands = {
    ...
    "delete-entities": function (player, cmd, data) {
        for (let ent of data.entities) {
            ...
            if (cmpMirage) {
                let cmpMiragedHealth = Engine.QueryInterface<Health>(cmpMirage.parent, IID_Health);
                if (cmpMiragedHealth)
            ...
            let cmpHealth = Engine.QueryInterface<Health>(ent, IID_Health);
            if (cmpHealth)
                cmpHealth.Kill();
            else
                Engine.DestroyEntity(ent);
...
function TryConstructWall(player, cmpPlayer, controlAllUnits, cmd) {
    ...
    for (; i < pieces.length; ++i) {
        ...
        var constructPieceCmd = {
            ...
            "obstructionControlGroup2": undefined,
        };
